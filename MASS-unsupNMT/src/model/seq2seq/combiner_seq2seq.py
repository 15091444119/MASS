...
class CombinerSeq2Seq(BaseSeq2Seq):
    ...
    def forward(self, loss_name, encoder_inputs, decoder_inputs):
        if loss_name == "explicit_loss":
             return self.explicit_loss(encode_inputs=encoder_inputs, decoder_inputs=decoder_inputs)
    ...
    def explicit_loss(self, encode_inputs, decoder_inputs, get_scores=False):
        ...
        Returns:
            scores, losses
        """
        encoded_info, combiner_loss, trained_combiner_words = self.encoder.explicit_encode_combiner_loss(encode_inputs=encode_inputs)
